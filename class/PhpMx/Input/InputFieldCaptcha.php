<?php

namespace PhpMx\Input;

use PhpMx\Cif;
use PhpMx\Mx5;
use Throwable;

/**
 * Campo de input para validação de captchas com cifra e hash.
 * Aplica automaticamente validação do código recebido contra a chave cifrada.
 */
class InputFieldCaptcha extends InputField
{
    /**
     * @param string $name Nome do campo.
     * @param string|null $alias Rótulo amigável para mensagens de erro.
     * @param mixed $value Valor inicial do campo (código captcha recebido).
     */
    function __construct(string $name, ?string $alias = null, mixed $value = null)
    {
        parent::__construct($name, $alias, $value);

        $this->required(true, 'Informe o código de validação');

        $this->sanitize(fn() => true);

        $this->validate(function ($recived) {
            try {
                $captcha = explode('|', "$recived|");
                $data = Cif::off($captcha[0]);
                $key = $data[0];
                $time = time() - $data[1];
                $value = mx5(strtoupper($captcha[1]));
                return Mx5::compare($key, $value) && $time <= env('CAPTCHA_TIME');
            } catch (Throwable $e) {
                return false;
            }
        }, 'Código de validação incorreto');
    }
}
